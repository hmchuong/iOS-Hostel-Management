//
//  GaiaDesignablePlaceholderTextView.swift
//  Holler
//
//  Created by Phong Le on 29/3/16.
...
@IBDesignable public class GaiaDesignablePlaceholderTextView: UITextView{
	// variables

	/** The string that will be put in the placeholder */
	@IBInspectable public var placeholder: String? { didSet { setNeedsDisplay() } }
	...
	func listenForTextChangedNotifications() {
		NotificationCenter.default.addObserver(self, selector: #selector(GaiaDesignablePlaceholderTextView.textChangedForPlaceholderTextView(_:)), name: NSNotification.Name.UITextViewTextDidChange, object: self)
		NotificationCenter.default.addObserver(self, selector: #selector(GaiaDesignablePlaceholderTextView.textChangedForPlaceholderTextView(_:)), name: NSNotification.Name.UITextViewTextDidBeginEditing, object: self)
	}
	...
	override public func draw(_ rect: CGRect) {
		...
		if self.text.isEmpty && self.placeholder != nil {
			let baseRect = placeholderBoundsContainedIn(CGRect(x: 5, y: 0, width: self.frame.width, height: self.frame.height))
	...
	func placeholderBoundsContainedIn(_ containerBounds: CGRect) -> CGRect {
		...
		let baseRect = UIEdgeInsetsInsetRect(containerBounds, UIEdgeInsetsMake(kPlaceholderTextViewInsetSpan, kPlaceholderTextViewInsetSpan / 2.0, 30, 0))
